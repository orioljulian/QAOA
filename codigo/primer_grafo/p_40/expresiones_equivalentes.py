import itertools

def are_equal(e1, e2, nbits=5, p=27):
    n = nbits
    is_wrong = False
    for z in itertools.product([-1, 1], repeat=n):
        vars = dict()
        for i in range(n):
            vars[f"z_{i}"] = z[i]

        res1 = eval(e1, dict({"P": p}, **vars))
        res2 = eval(e2, vars)
        if res1 != res2:
            is_wrong = True
            print("Error: ", f"{z}", res1, res2)
    if not is_wrong:
        print("Las expresiones son equivalentes")


exp_orig = "\
5*(1 - z_0)/2 + \
8*(1 - z_1)/2 + \
2*(1 - z_2)/2 + \
7*(1 - z_3)/2 + \
4*(1 -  z_4)/2 + \
P * ((1 - z_0)/2 + (1 - z_1)/2 - 1)**2 + \
P * ((1 - z_0)/2 - (1 - z_2)/2 - (1 - z_3)/2)**2 + \
P * ((1 - z_1)/2 + (1 - z_2)/2 - (1 - z_4)/2)**2"

exp_40 = "\
17.5*z_0 - 24*z_1 - 41*z_2 - 23.5*z_3 + 18*z_4 \
+ 20*z_0*z_1 - 20*z_0*z_2 - 20*z_0*z_3 + 20*z_1*z_2 - 20*z_1*z_4 \
    + 20*z_2*z_3 - 20*z_2*z_4 \
+ 20*z_0**2 + 20*z_1**2 + 20*z_2**2 + 10*z_3**2 + 10*z_4**2 + 33 \
"

if __name__ == "__main__":
    # Sin restriccion extra
    print("exp_orig == exp_40: ", end="")
    are_equal(exp_orig, exp_40, p=40)

    # Con restriccion extra
    print("exp_orig_mas_restricc == exp_40_mas_restricc: ", end="")
    are_equal(exp_orig_mas_restricc, exp_40_mas_restricc, p=40)
