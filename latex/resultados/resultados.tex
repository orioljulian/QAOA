...
\label{sec:org342fc88}
\subsubsection{Modificadores paper}
\label{sec:orga622b98}
\lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
circuit.rz(coef, q_idx)
circuit.rzz(coef * gamma[p], q_idxs[0], q_idxs[1])
...
\subsection{Provider}
\label{sec:orgfd53a40}
\subsubsection{ibmq\textunderscore lima}
\label{sec:org90fc840}
...
\subsection{Runtime}
\label{sec:org5240972}
\subsubsection{ibmq\textunderscore lima}
\label{sec:org4bec217}
\(\beta\)\textsubscript{0} = 0.5, \(\gamma\)\textsubscript{0} = 0.5
\begin{center}
\begin{tabular}{|r|l|r|}
\hline
